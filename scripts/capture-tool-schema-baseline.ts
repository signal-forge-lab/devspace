...
} from "../src/workbridge-tool-policy.js";
...
try {
  ...
  const snapshot = {
    ...
    configuration: {
      toolSurface: "upstream-codex-plus-workbridge",
      upstreamToolMode: WORKBRIDGE_UPSTREAM_TOOL_MODE,
      widgets: WORKBRIDGE_WIDGET_MODE,
      reviewTool: WORKBRIDGE_REVIEW_TOOL_NAME,
      workbridgeExtensions: [...WORKBRIDGE_EXTENSION_TOOL_NAMES],
      runtimeProfileSwitching: WORKBRIDGE_DISABLED_CAPABILITIES.includes("runtime-profile-switching")
        ? "disabled"
        : "enabled",
      commandMetadata: "always",
      skills: "always",
      subagents: WORKBRIDGE_SUBAGENTS_ENABLED ? "enabled" : "disabled",
      disabledCapabilities: [...WORKBRIDGE_DISABLED_CAPABILITIES],
