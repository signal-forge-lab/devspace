...
- 状態: **Workbridge独自transport制限を廃止（2026-07-22）**
- commit: `5654110 fix bound transport and workspace session lifetime`
- 当初のWorkbridge独自実装:
  - MCP transportを既定最大32件に制限
  - transport idleをupstreamの24時間から1時間へ短縮
  - 最大数到達時は最も古いtransportをcloseして入れ替え
- 廃止理由:
  - ChatGPTが短時間に多数のMCP sessionを生成する環境では、32件到達時のcapacity evictionが利用中の会話sessionをcloseし、既存チャットからWorkbridgeが利用不能になる可能性が高いことを実ログで確認した
- 2026-07-22の変更:
  - `DEVSPACE_MCP_MAX_TRANSPORTS`を削除
  - `DEVSPACE_MCP_TRANSPORT_IDLE_SECONDS`を削除
  - capacity到達時の最古transport強制closeを削除
  - upstreamと同じ24時間idle cleanupを復元
  - upstreamと同じserver shutdown時の全transport closeを維持
  - 64件を登録してもcapacity evictionしない回帰テストを追加
  - active session分類、MCP method、client/protocol、RSS/heapを5分ごとにログ化
  - active sessionが128件・512件を超えた際の警告を追加
  - initialize、GET SSE、`notifications/initialized`だけで止まったpre-use sessionを15分でclose
  - `tools/call`到達session数、複数tool callへ再利用されたsession数、1session当たり最大tool call数を計測
  - discoveryまたはoperational requestへ進んだsessionは件数を理由にcloseせず、24時間idleを維持
- 維持する独立機能:
  - SQLiteのworkspace session recordを既定30日で起動時整理
  - 設定: `DEVSPACE_WORKSPACE_SESSION_MAX_AGE_DAYS`
...
| `5654110` | 一部廃止 | transport capacity制限・1時間idleは廃止。workspace sessionの30日整理のみ維持 |
