...
Workbridge can save a file attached or generated by an MCP host, such as ChatGPT,
directly into an open workspace. `download_artifact` is always part of the fixed
tool surface; no enable flag changes its visibility.
...
owner-only files without overwriting an existing destination. Execution is
currently supported on Linux. On macOS, Windows, and BSD the tool remains
registered for a stable MCP contract and returns an explicit unsupported-
