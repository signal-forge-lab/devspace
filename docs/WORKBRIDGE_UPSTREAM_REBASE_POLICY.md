...
Workbridge package versions use the upstream DevSpace release as the base and a
Workbridge-specific prerelease suffix:

```text
<upstream-version>-workbridge.<revision>
```

For example, the first Workbridge build based on upstream `1.0.6` is
`1.0.6-workbridge.1`. Increment the Workbridge revision for downstream-only
releases that stay on the same upstream base. Reset it to `1` when rebasing onto
a new upstream release. The package version is used for runtime display,
managed-restart verification, and stored tool-contract baseline metadata; it is
not a workspace-session or database schema identifier.
...
Keep upstream `allowedRoots` as the project-root contract. Workbridge-specific
auxiliary checkout roots belong in `src/workbridge-root-policy.ts` and should be
integrated through small hooks rather than renaming or replacing the upstream
root model. Managed worktree sources remain restricted to upstream project roots,
and the configured managed-worktree destination must stay beneath a project root.
...
After an accepted change is integrated, do not retain its managed worktree as
an informal archive. Run `npm run maintenance:worktrees` to identify integrated
managed worktrees. Preserve still-needed unmerged commits with their branch; if
a clean detached HEAD has no protecting ref, create a local
`refs/archive/worktrees/YYYYMMDD/<worktree-name>` ref before removing the
worktree directory. The same maintenance audit also identifies integrated local
branches that are no longer checked out or protected by an upstream tracking
branch; remove those branch names after preserving any intentionally retained
history under a local archive ref.
...
- the upstream Codex/review tool policy remains selected, Workbridge-owned tool
  additions remain explicit, and every model-visible change is reviewed through
  the schema baseline;
...
- OAuth, allowed-root enforcement, path redaction, and artifact boundaries remain
  intact;
- Session Monitor and Soft Pause integration do not independently broaden the
  public tool surface.
