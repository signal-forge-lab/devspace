...
Workbridge exposes local coding capabilities over MCP. Treat it as remote access
...
Workbridge only opens workspaces under configured roots.
...
When an MCP client connects, Workbridge shows an approval page. Enter the Owner
...
Workbridge needs `DEVSPACE_PUBLIC_BASE_URL` so MCP clients can discover OAuth
...
By default, Workbridge derives allowed Host headers from the local host and public
URL. Use `DEVSPACE_ALLOWED_HOSTS=*` only for intentional local debugging.

## Tunnels

Workbridge does not manage tunnels. Your tunnel or reverse proxy should point to:
...
protection in front of public tunnels. Workbridge OAuth still protects the MCP
...
Filesystem path containment applies to Workbridge file tools. Shell commands run
...
When `DEVSPACE_OAUTH_SAFE_DIAGNOSTIC_LOGGING` is enabled, Workbridge logs only OAuth routing metadata needed for connector setup, such as `client_id`, `redirect_uri`, `redirect_uris`, scope, and rejection reason. It must not log authorization codes, access tokens, refresh tokens, client secrets, cookies, or Authorization headers.
