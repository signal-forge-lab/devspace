...
`workbridge-codex-tool-schema.json` records the reviewed model-visible MCP
contract. It is generated through an in-memory MCP client/server exchange and
contains:

- the current upstream Codex/review tool names plus Workbridge extensions
- one SHA-256 digest per tool, calculated from the complete canonical tool
  contract returned by `tools/list`
- the selected surface policy and disabled capabilities
...
Update it only when a model-visible tool contract change is intentional. The
baseline detects upstream additions and schema changes for review; it is not a
runtime allowlist that rejects a tool solely because the count changed.
