...
try {
  ...
  assert.equal(workbridge.command, "npm run verify:rebase && git status --short");
  ...
  await mkdir(join(chromeRoot, "images"), { recursive: true });
  await writeFile(
    ...
    JSON.stringify({
      ...
      web_accessible_resources: [{
        resources: ["pages/public.html", "images/*.png"],
        matches: ["https://example.com/*"],
      }],
    }),
  );
  ...
  await Promise.all([
    ...
    writeFile(join(chromeRoot, "pages", "public.html"), "\n"),
    writeFile(join(chromeRoot, "images", "public.png"), "\n"),
  ]);
  ...
  const directoryResourceRoot = join(root, "chrome-directory-resource");
  await mkdir(join(directoryResourceRoot, "scripts"), { recursive: true });
  await writeFile(
    join(directoryResourceRoot, "manifest.json"),
    JSON.stringify({
      manifest_version: 3,
  ...
  await assert.rejects(
    () => resolveProjectVerifyProfile({ workspaceRoot: directoryResourceRoot }),
    (error: unknown) => {
      assert.ok(error instanceof ProjectProfileResolutionError);
      assert.equal(error.kind, "missing_extension_resource");
      assert.match(error.message, /scripts/);
      return true;
    },
  );

  const missingWebAccessibleRoot = join(root, "chrome-missing-web-accessible-resource");
  await mkdir(missingWebAccessibleRoot, { recursive: true });
  await writeFile(
    join(missingWebAccessibleRoot, "manifest.json"),
    JSON.stringify({
      manifest_version: 2,
      name: "Missing web accessible resource",
      version: "1.0.0",
      web_accessible_resources: ["missing/*.js"],
    }),
  );
  await assert.rejects(
    () => resolveProjectVerifyProfile({ workspaceRoot: missingWebAccessibleRoot }),
    (error: unknown) => {
      assert.ok(error instanceof ProjectProfileResolutionError);
      assert.equal(error.kind, "missing_extension_resource");
