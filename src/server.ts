...
interface ToolWidgetDescriptorMeta extends Record<string, unknown> {
...
export function createMcpServer(
  ...
  const { registerTool, artifactRegisterTool } = createWorkbridgeToolRegistrars({
    softPause,
    workspaces,
    monitorContext,
  });
  const server = new McpServer(
    ...
    {
      instructions: workbridgeServerInstructions(),
    },
  );
  ...
  registerWorkbridgeExtensionTools({
    ...
    shellToolMeta: toolWidgetDescriptorMeta(config, "shell"),
  });
