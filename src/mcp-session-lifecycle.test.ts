...
await testDefaultOneShotGraceIsSixtySeconds();
...
async function testDefaultOneShotGraceIsSixtySeconds(): Promise<void> {
  assert.equal(MCP_ONE_SHOT_GRACE_MS, 60_000);
  let now = 0;
  const transport = new FakeTransport();
  const registry = new McpSessionRegistry<FakeTransport>({ now: () => now });
  registry.register("one-shot", transport, {}, { oneShotCleanupEligible: true });
  assert.equal(registry.beginRequest("one-shot", ["tools/call"]), transport);
  assert.equal(registry.endRequest("one-shot"), true);
  const lifecycle = new McpSessionLifecycle({ registry });
