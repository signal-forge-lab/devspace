...
export function workbridgeConsoleLine(
  event: string,
  fields: LogFields,
): string | undefined {
  return compactToolCallConsoleLine(fields)
    ?? compactHttpRequestConsoleLine(event, fields)
    ?? compactMcpSessionConsoleLine(event, fields);
}

export function publishWorkbridgeConsoleLine(
