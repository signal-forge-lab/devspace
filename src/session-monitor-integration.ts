...
import {
  recordMonitorOperationDetails,
  runMonitorOperation,
  sanitizeMonitorText,
} from "./monitor-operation-context.js";
import { sessionMonitorHtml } from "./session-monitor-ui.js";
import { buildMonitorWorkspaceContext } from "./workbridge-workspace-context.js";
import type { WorkspaceRegistry } from "./workspaces.js";
import type { McpSessionSnapshot } from "./mcp-sessions.js";
import type { ModernMcpRequestSnapshot } from "./mcp-modern-metrics.js";
import type { NodeSaturationSnapshot } from "./node-saturation-metrics.js";
import type { SoftPauseState } from "./soft-pause.js";
import type { WorkbridgeConfigProvenance } from "./workbridge-config-provenance.js";
import type { WorkbridgeRuntimeBuildIdentity } from "./workbridge-runtime-identity.js";
import type { WorkbridgeMcpConnectionMode } from "./config.js";
...
export interface SessionMonitorRuntimeStatus {
  server: {
    ...
    buildIdentity: WorkbridgeRuntimeBuildIdentity;
    configProvenance: WorkbridgeConfigProvenance;
    startupConfig: {
  ...
  modernMcpRequests: ModernMcpRequestSnapshot;
  nodeSaturation: NodeSaturationSnapshot;
...
export function createSessionMonitorToolRegistrar(
  ...
  return ((server, name, definition, handler) => {
    const monitoredHandler = (async (...args: unknown[]) => {
      ...
      return await runMonitorOperation(
        operationId,
        (details) => context.monitor.updateTool(reference, details),
        async () => {
          try {
            ...
            context.monitor.completeTool(
              ...
              result,
              resultWorkspaceId
                ? workspaceIdentityForId(workspaces, resultWorkspaceId, result)
                : undefined,
            );
            return result;
          } catch (error) {
            recordMonitorOperationDetails({
              error: sanitizeMonitorText(error instanceof Error ? error.message : String(error)),
            });
...
export function registerSessionMonitorRoutes(
  ...
  app.get("/monitor/api/operations/:operationId", (req, res) => {
    if (!isLocalMonitorRequest(req)) {
      res.status(404).end();
      return;
    }
    res.setHeader("Cache-Control", "no-store");
    const operation = monitor.operation(req.params.operationId);
    if (!operation) {
      res.status(404).json({ ok: false, error: "Operation not found" });
      return;
    }
    res.json({ version: 1, generatedAt: Date.now(), operation });
  });
...
function workspaceIdentityForId(
  ...
  try {
    ...
    return {
      ...
      workspaceContext: buildMonitorWorkspaceContext(workspace, result),
...
export function isLocalMonitorRequest(req: Request): boolean {
  if (
    req.header("cf-connecting-ip") !== undefined
